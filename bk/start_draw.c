...
/*   Updated: 2021/10/18 17:33:34 by dpavon-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	//ft_printf("%d, %d\n", dates->y0, dates->x0);
	dates->z0 = (maptrix[dates->y0][dates->x0].number * 2);
	dates->z1 = (maptrix[dates->y1][dates->x1].number * 2);
	dates->x = dates->x0;
	dates->y = dates->y0;
...
void	to_write(t_values **maptrix, t_vars *mlx, t_bresenham *n, int col)
{	
	...
	n->position_line += 1;
	diff = maptrix[n->y][n->x].color - col;
	total = diff / n->increment * n->position_line;

	if (n->x0 >= 0 && n->x0 < 1500 && n->y0 >= 0 && n->y0 < 1500)
		my_mlx_pixel_put(mlx, n->x0, n->y0, maptrix[n->y][n->x].color - total);
	// n->position_line++;
	(void)maptrix;
	(void)col;
}
...
void	write_algorithm(t_values **maptrix, t_vars *mlx, t_bresenham *n)
{
	...
	if (maptrix[n->y1][n->x1].number >= 10)
		maptrix[n->y1][n->x1].color = 15950335;
	col = maptrix[n->y1][n->x1].color;
